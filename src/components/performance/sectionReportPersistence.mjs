...
export async function confirmSectionReportSave(db, { mode, action, cycle, userId, report, summary, feedback }) {
  ...
  try {
    const result = mode === 'compose'
      ...
      : await db.rpc('review_performance_section_report', { p_id: report?.id, p_action: action, p_feedback: feedback, p_expected_updated_at: report?.updated_at });
  ...
  if (!saved || saved.updated_at === report?.updated_at || saved.status !== status ||
      (mode === 'compose' ? saved.summary !== summary : saved.director_feedback !== feedback)) {
