-- Fix the system completion status function with correct station progression logic
  ...
  next_station AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN EXISTS (SELECT 1 FROM station_completion WHERE station_progress < 100 AND station_order = 4) THEN 'Station 4'
        -- If all stations are complete, stay at Station 4
        ELSE 'Station 4'
    ...
    WHERE station_order BETWEEN 0 AND 4
    GROUP BY system_id
  )
  UPDATE test_systems
  SET 
    ...
    status = CASE 
      WHEN COALESCE(ns.completed_stations, 0) = 5 THEN 'Done'
