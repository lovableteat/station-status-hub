-- user_notifications.reference_id is text while performance_reviews.id is uuid.
-- Cast the review ID before comparing or storing it so the return notification
-- function works after a review has already moved back to in-progress.
...
begin
  ...
  where notification.recipient_id = v_recipient
    ...
    and notification.reference_id = p_review_id::text
