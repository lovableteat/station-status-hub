-- Shared tools, uploaded files, code snippets, and commands.
-- General uploaded files already live in tools_management, so they share its scope.

alter table workspace.tools_management
  add column if not exists scope text not null default 'global',
  add column if not exists owner_workspace text;

update workspace.tools_management
set scope = 'global'
where scope is null;

update workspace.tools_management
set owner_workspace = null
where scope = 'global'
  and owner_workspace is not null;
...
alter table workspace.tools_management
  drop constraint if exists tools_management_scope_owner_check;

alter table workspace.tools_management
  add constraint tools_management_scope_owner_check
  check (
    (scope = 'global' and owner_workspace is null)
    or (
      scope = 'workspace'
      and nullif(btrim(owner_workspace), '') is not null
    )
  );

create index if not exists idx_tools_management_scope_owner_workspace
  on workspace.tools_management(scope, owner_workspace);

alter table workspace.code_snippets
  add column if not exists scope text not null default 'global',
  add column if not exists owner_workspace text;

update workspace.code_snippets
set scope = 'global'
where scope is null;
...
alter table workspace.code_snippets
  drop constraint if exists code_snippets_scope_owner_check;

alter table workspace.code_snippets
  add constraint code_snippets_scope_owner_check
  check (
    (scope = 'global' and owner_workspace is null)
    or (
      scope = 'workspace'
...
create index if not exists idx_code_snippets_scope_owner_workspace
  on workspace.code_snippets(scope, owner_workspace);

alter table workspace.command_library
  add column if not exists scope text not null default 'global',
  add column if not exists owner_workspace text;

update workspace.command_library
set scope = 'global'
where scope is null;
...
alter table workspace.command_library
  drop constraint if exists command_library_scope_owner_check;

alter table workspace.command_library
  add constraint command_library_scope_owner_check
  check (
    (scope = 'global' and owner_workspace is null)
    or (
      scope = 'workspace'
...
create index if not exists idx_command_library_scope_owner_workspace
  on workspace.command_library(scope, owner_workspace);

grant select, insert, update, delete on table workspace.tools_management to anon, authenticated, service_role;
grant select, insert, update, delete on table workspace.code_snippets to anon, authenticated, service_role;
grant select, insert, update, delete on table workspace.command_library to anon, authenticated, service_role;

-- Replace the archived three-argument overload so existing callers resolve to
-- the new function and receive the station-status workspace default.
drop function if exists workspace.search_maintenance_knowledge(text, uuid[], integer);

create or replace function workspace.search_maintenance_knowledge(
  p_query text,
  p_project_ids uuid[],
  p_limit integer default 16,
  p_workspace text default 'station-status'
)
returns table (
  chunk_id text,
  source_type text,
...
stable
security definer
set search_path = workspace, extensions, auth, pg_temp
as $$
declare
  ...
  v_workspace text := btrim(coalesce(p_workspace, 'station-status'));
...
begin
  ...
  if v_user.role in ('admin', 'super_admin') then
    ...
    if coalesce(v_user.permissions, '{}'::jsonb) #> '{workspaceAccess}' ? 'station-status' then
      ...
      select exists (
        select 1
        from workspace.user_page_permissions as permissions
  ...
  station_chunks as (
    ...
    left join lateral (
      ...
      from workspace.test_flow_items as flow_items
    ...
    left join lateral (
      ...
      from workspace.station_contents as station_content
  ...
  progress_chunks as (
    ...
    join workspace.test_systems as systems on systems.id = progress.system_id
    join workspace.test_flow_stations as stations on stations.id = progress.station_id
    join workspace.test_flow_items as flow_items on flow_items.id = progress.item_id
  ...
  issue_chunks as (
    ...
    left join workspace.test_systems as systems on systems.id = issues.system_id
    left join workspace.test_flow_stations as stations on stations.id = issues.station_id
    left join workspace.test_flow_items as flow_items on flow_items.id = issues.test_item_id
  ),
  asset_chunks as (
    select
    ...
    from workspace.test_project_tool_assignments as assignments
    join selected_projects as projects on projects.id = assignments.project_id
    join workspace.tools_management as tools on tools.id = assignments.tool_id
    where (
      tools.scope = 'global'
      or (
        tools.scope = 'workspace'
        and tools.owner_workspace = v_workspace
      )
    )
    ...
    select
      ...
      7.0::real
    from workspace.test_project_code_assignments as assignments
    join selected_projects as projects on projects.id = assignments.project_id
    join workspace.code_snippets as snippets on snippets.id = assignments.code_snippet_id
    where (
      snippets.scope = 'global'
      or (
        snippets.scope = 'workspace'
        and snippets.owner_workspace = v_workspace
      )
    )
    ...
    select
      ...
      7.0::real
    from workspace.test_project_command_assignments as assignments
    join selected_projects as projects on projects.id = assignments.project_id
    join workspace.command_library as commands on commands.id = assignments.command_id
    where commands.is_active = true
      and (
        commands.scope = 'global'
        or (
          commands.scope = 'workspace'
          and commands.owner_workspace = v_workspace
        )
      )
  ),
  chunks as (
...
revoke all on function workspace.search_maintenance_knowledge(text, uuid[], integer, text) from public;
grant execute on function workspace.search_maintenance_knowledge(text, uuid[], integer, text) to anon, authenticated, service_role;

-- Keep the legacy public RPC entry point aligned for deployments that have not
-- yet switched their PostgREST schema profile to workspace.
drop function if exists public.search_maintenance_knowledge(text, uuid[], integer);
...
as $$
  select *
  from workspace.search_maintenance_knowledge(
    p_query,
    p_project_ids,
    p_limit,
    p_workspace
...
revoke all on function public.search_maintenance_knowledge(text, uuid[], integer, text) from public;
grant execute on function public.search_maintenance_knowledge(text, uuid[], integer, text) to anon, authenticated, service_role;
