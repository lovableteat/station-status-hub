...
-- Insert test items for each station
INSERT INTO public.test_flow_items (station_id, item_name, item_order, description, estimated_minutes) 
SELECT s.id, item_data.item_name, item_data.item_order, item_data.description, item_data.estimated_minutes
FROM public.test_flow_stations s
CROSS JOIN (
  SELECT 'Chassis Assembly' as item_name, 1 as item_order, '組裝機殼' as description, 120 as estimated_minutes WHERE s.station_name = 'Station 0'
  UNION ALL SELECT 'Tim Curing', 2, '加熱至 65°C，持續 60 分鐘', 180 WHERE s.station_name = 'Station 0'
  UNION ALL SELECT 'Boot to BIOS', 1, '開機至 UEFI', 30 WHERE s.station_name = 'Station 1'
  UNION ALL SELECT 'Monitor Display', 2, 'PCI 顯示 PCIE 裝置', 30 WHERE s.station_name = 'Station 1'
  UNION ALL SELECT 'BIOS Update', 1, 'BIOS 韌體更新', 20 WHERE s.station_name = 'Station 2'
  UNION ALL SELECT 'BMC Update', 2, 'BMC 韌體更新', 20 WHERE s.station_name = 'Station 2'
  UNION ALL SELECT 'GPU Update', 3, 'GPU 韌體更新', 15 WHERE s.station_name = 'Station 2'
  UNION ALL SELECT 'FRU/MAC Update', 4, 'FRU / MAC 更新', 25 WHERE s.station_name = 'Station 2'
  UNION ALL SELECT 'PCIe Check', 1, 'PCIe 裝置檢查 lspci', 15 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'GPU Verification', 2, 'GPU 驗證 nvidia-smi', 15 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'Network Test', 3, 'BF3 / CX8 狀態查詢', 20 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'Infiniband Test', 4, 'Infiniband 測試 ibstat', 15 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'TPM Check', 5, 'TPM、I210 MAC 驗證', 15 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'SFT Test', 6, 'SFT 測試', 20 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'Stress Test', 7, 'CPU / DIMM / Disk 壓力測試', 30 WHERE s.station_name = 'Station 3'
  UNION ALL SELECT 'Tim Bake Tool', 1, 'Tim Bake 工具執行', 60 WHERE s.station_name = 'Station 4'
  UNION ALL SELECT 'NV Diag', 2, 'NV 協同測試項目', 120 WHERE s.station_name = 'Station 4'
  UNION ALL SELECT 'Log Collection', 3, 'Dump Log 並標記 MFG mode', 30 WHERE s.station_name = 'Station 4'
) item_data
WHERE s.station_name = CASE 
  WHEN item_data.item_name IN ('Chassis Assembly', 'Tim Curing') THEN 'Station 0'
  WHEN item_data.item_name IN ('Boot to BIOS', 'Monitor Display') THEN 'Station 1' 
  WHEN item_data.item_name IN ('BIOS Update', 'BMC Update', 'GPU Update', 'FRU/MAC Update') THEN 'Station 2'
  WHEN item_data.item_name IN ('PCIe Check', 'GPU Verification', 'Network Test', 'Infiniband Test', 'TPM Check', 'SFT Test', 'Stress Test') THEN 'Station 3'
  WHEN item_data.item_name IN ('Tim Bake Tool', 'NV Diag', 'Log Collection') THEN 'Station 4'
END;
