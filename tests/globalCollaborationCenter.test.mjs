...
test("the workspace keeps direct messages exclusively in the bottom-right floating panel", async () => {
  ...
  assert.match(centerSource, /data-chat-launcher="floating-pill"/);
  assert.match(centerSource, /w-\[min\(310px,100%\)\]/);
  ...
  assert.match(centerSource, /bg-\[linear-gradient\(125deg,#e4fcff/);
